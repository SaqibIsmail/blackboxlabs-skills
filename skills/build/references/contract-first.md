...
> **Provenance note (not part of the original file):** copied verbatim from `affaan-m/ECC`,
> `skills/contract-first/SKILL.md` (MIT), fetched 2026-09-19. Kept unfiltered — this skill is
> stack-agnostic (it lists OpenAPI, AsyncAPI, Protocol Buffers, JSON Schema, and a shared typed
> interface as interchangeable boundary-artifact choices, picked per-boundary), unlike the ECC
> pattern files in this same directory that got filtered against `blackboxlabs`'s own locked
> stack. One addition was made instead of a filter pass: the "Applied to this project" section
> at the end, naming which of this skill's own boundary-artifact options this single-repo
> Next.js codebase actually uses, per the skill's own rule ("a typed interface only when every
> participant shares the same build and runtime compatibility model"). That wording was agreed
> in `.claude/plans/graceful-orbiting-oasis.md` (see its "API + contract" step) and is reproduced
> here, not paraphrased fresh. Regenerate the base content above from upstream if it changes;
> the appended section is this project's own and stays.
...
**(Added for `blackboxlabs`, not part of the original ECC skill above.)**

`blackboxlabs` is a single Next.js repo where frontend and backend share one
TypeScript build — not separate services. Applying this skill's own rule from
"The Boundary Artifact" above ("a typed interface only when every participant
shares the same build and runtime compatibility model"), the canonical
artifact for an API/data boundary here is a shared TypeScript type in
`features/shared/`, not OpenAPI, AsyncAPI, or JSON Schema.

Concretely, when a task's "API + contract" check finds two tickets (frontend
and backend) touching the same resource:

- Grep `features/shared/` for the type first.
- If it's missing, the ticket touching this resource *first* (per
  senior-engineer's no-forward-dependencies ordering) creates it, and the
  sibling ticket imports it rather than re-declaring its own shape.
- Drift is then caught for free by the project's own `pnpm typecheck` gate the
  moment either side diverges from the shared type — no separate
  contract-check process needed on top of it.

This is a narrowing of the skill's menu of options to the one this project's
architecture actually supports, not a change to the skill's own criterion for
choosing it.
