...
MADR-minimal. One file per decision at `<vault_path>/<decisions_subpath>/NNNN-<slug>.md` (the
numbering convention borrowed from `adr-tools`, without adopting its unmaintained CLI):
...
ticket-refs: [FE-123, BE-124]     # filled in later by assign-tasks once tickets exist
tags: [...]
---
...
| Stage | Writes | Reads |
|---|---|---|
| `plan-feature` | Yes — first entry when `plan.md` finalizes (scope, chosen approach, explicit non-goals) | On resume, if this feature already has entries |
| `assign-tasks` | Enriches the existing entry with `ticket-refs` once tickets exist | — |
...
2. `vault_path` has no default and almost certainly lives outside the target repo (a personal
   Obsidian vault, not something cloned per-project). First real run needs to handle "vault
   doesn't exist yet at this path" gracefully — create it, or fail with clear instructions?
