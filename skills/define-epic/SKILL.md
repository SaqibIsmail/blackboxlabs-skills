...
**API note (confirmed live 2026-09-13):** Jira's REST API v3 `/rest/api/3/search` endpoint is
deprecated (returns `410 Gone`) — use `/rest/api/3/search/jql` instead. `jira-integration`'s own
docs still reference the old endpoint; if `jira_search`'s underlying implementation hasn't been
updated, fall back to calling `/rest/api/3/search/jql` directly.
...
**Use real Atlassian Document Format structure, not a wall of prose** — headings (`Why`, `Scope`)
with bullet lists underneath, bold labels (`Who:`, `Current:`, `Desired:`, `Why now:`, `Done
when:`, `Out of scope:`, `Touches:`, `MVP:`, etc.), not one dense paragraph per section:
